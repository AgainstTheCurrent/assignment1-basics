...
import cs336_basics.pytorch.modules as modules
from cs336_basics.pytorch.nn_utils import gradient_clipping, softmax, scaled_dot_product_attention, cross_entropy
...
def get_qkv_weight(
    ...
    return torch.cat([q_proj_weight, k_proj_weight, v_proj_weight], dim=-2)
...
def run_multihead_self_attention(
    ...
    """
    mhsa = modules.MultiHeadSelfAttention(d_model=d_model, num_heads=num_heads)
    # Concatenate q, k, v projection weights along dim=0 to form (3 * d_model, d_model)
    # Each should be (num_heads * d_k, d_in), so stack and reshape
    mhsa.weight_qkv = torch.nn.Parameter(get_qkv_weight(q_proj_weight, k_proj_weight, v_proj_weight))
    mhsa.weight_o = torch.nn.Parameter(o_proj_weight)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    mhsa = modules.MultiHeadSelfAttention(d_model=d_model, num_heads=num_heads, rope=True, theta=theta, max_seq_len=max_seq_len)
    # Concatenate q, k, v projection weights along dim=0 to form (3 * d_model, d_model)
    # Each should be (num_heads * d_k, d_in), so stack and reshape
    mhsa.weight_qkv = torch.nn.Parameter(get_qkv_weight(q_proj_weight, k_proj_weight, v_proj_weight))
    mhsa.weight_o = torch.nn.Parameter(o_proj_weight)
...
def run_transformer_block(
    ...
    # Assign weights
    block.attn.weight_qkv.data = get_qkv_weight(weights["attn.q_proj.weight"], weights["attn.k_proj.weight"], weights["attn.v_proj.weight"])
    block.attn.weight_o.data = weights["attn.output_proj.weight"]
...
def run_transformer_lm(
    ...
    for layer in range(num_layers):
        ...
        block.attn.weight_qkv.data = get_qkv_weight(
            weights[prefix + "attn.q_proj.weight"],
            weights[prefix + "attn.k_proj.weight"],
            weights[prefix + "attn.v_proj.weight"])
        block.attn.weight_o.data = weights[prefix + "attn.output_proj.weight"]
...
def run_rmsnorm(
    ...
    """
    rmsnorm = modules.RMSNorm(d_model=d_model, eps=eps)
...
def run_load_checkpoint(
    ...
    """
    return training_utils.load_checkpoint(src, model, optimizer)
